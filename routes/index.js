...
router.get('/allBalance',  wrapAsync(async function (req, res, next) {
...
// 上架商品
router.post('/Info',  wrapAsync(async function (req, res, next) {
	if (!!req.session.name) {
		setKeytoWeb3(req.session.privatekey);
...
// 目前商品編號
router.get('/currentProductNum',  wrapAsync(async function (req, res, next) {
	let bank = new web3.eth.Contract(contract.abi);
	bank.options.address = req.query.address;
	let productnum = await bank.methods.currentProductNum().call()
...
// 取得合約內所有的商品
router.get('/products',  wrapAsync(async function (req, res, next) {
	let bank = new web3.eth.Contract(contract.abi);
	bank.options.address = req.query.address;
...
// 當查看商品資訊
router.get('/Info',  wrapAsync(async function (req, res, next) {
	let bank = new web3.eth.Contract(contract.abi);
	bank.options.address = req.query.address;
...
//取得目前交易狀態
router.get('/TradeStatus',  wrapAsync(async function (req, res, next) {
	let bank = new web3.eth.Contract(contract.abi);
	bank.options.address = req.query.address;
...
//設定交易狀態
router.post('/TradeStatus', wrapAsync( async function (req, res, next) {
	let bank = new web3.eth.Contract(contract.abi);
	bank.options.address = req.body.address;
	if (!!req.session.name) {
		setKeytoWeb3(req.session.privatekey);
...
//紀錄買家遊戲ID
router.post('/BuyerID',  wrapAsync(async function (req, res, next) {
	let bank = new web3.eth.Contract(contract.abi);
	bank.options.address = req.body.address;
	if (!!req.session.name) {
		setKeytoWeb3(req.session.privatekey);
...
//取得買家紀錄在合約內的遊戲ID
router.get('/BuyerId',  wrapAsync(async function (req, res, next) {
	let bank = new web3.eth.Contract(contract.abi);
	bank.options.address = req.query.address;
	let flag = 0;
	if (!!req.session.name) {
...
// 購買鍵 買家支付至合約端
router.post('/buy',  wrapAsync(async function (req, res, next) {
	let bank = new web3.eth.Contract(contract.abi);
	let erc20 = new web3.eth.Contract(tokenContract.abi);
	bank.options.address = req.body.address;
...
router.get('/successtrade',  wrapAsync(async function (req, res, next) {
	let bank = new web3.eth.Contract(contract.abi);
	let erc20 = new web3.eth.Contract(tokenContract.abi);
	bank.options.address = req.query.address;
	erc20.options.address = req.query.erc20Address;
...
//買家領收商品 合約將款項交付給賣家
router.post('/withdraw',  wrapAsync(async function (req, res, next) {
	let bank = new web3.eth.Contract(contract.abi);
	bank.options.address = req.body.address;
...
router.post('/refund',  wrapAsync(async function (req, res, next) {
	let bank = new web3.eth.Contract(contract.abi);
	bank.options.address = req.body.address;
	let value = bank.methods.getProductPrice(req.body.nowProduct).call();
